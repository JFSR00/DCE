...
ISR(TIMER0_COMPA_vect){				// Sound generation routine on compare match interrupt vector
	BUZZER_PORT^=(1<<BUZZER_PIN);	// Toggles buzzer's pin
}
...
void timer0_init(uint8_t ocr, uint16_t prescaler){
	BUZZER_DDR |= (1 << BUZZER_PIN);	// Output pin for the buzzer
...
void beep(uint16_t freq, float period){
	timer0_init(OCR(freq, TIMER0_PRESC), TIMER0_PRESC);	// That sets the values for the sound generation at the desired frequency
	timer1_init();										// Here we configure the Timer 1 to be use for making delays
	
	timer0_enaINT();									// That function enables the sound reproduction
	sei();
	timer1_delay_ms(period);
	cli();
	timer0_disINT();									// That function disables the sound reproduction
}